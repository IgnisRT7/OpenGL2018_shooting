...
namespace Application {
	...
	MoveControllerPtr MakeMoveControllerByMoveType(int type, bool inverse) {
		...
		case 3:		// left→right (stair)
			seq->Add(std::make_shared<MoveStraight>(6.f, glm::vec3(GameState::screenSizeW * 1.2f * (inverse ? 1 : -1), 0, 0)));
			break;
		case 4:
		...
		case 5:
			...
			seq->Add(std::make_shared<MoveStraight>(2.f, glm::vec3(0, 0, 0)));
			seq->Add(std::make_shared<MoveStraight>(4.f, glm::vec3(0, 0, -GameState::screenHalfH * 2)));
			break;
		case 6:		//circle
	...
	void EnemyLaunchController::Update(float deltaTime) {
		...
		while (true) {
			...
			if (itr->launchStartTimer < timer) {
				...
				auto e = std::make_shared<EnemySpawner>(
